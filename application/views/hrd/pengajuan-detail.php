...
<div class="row">
    ...
    <div class="col-lg-4 col-xl-3">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title m-b-15">Pegawai</h6>
                    ...
                    <?php 
                        if( $row->approve_by != null){
                            echo ' <div class="form-group">
                                   ';
                            if($row->approve == 'y'){
                                echo ' <label for="">Disetujui oleh</label><h6>';
                            }else if($row->approve == 'n'){
                                echo ' <label for="">Ditolak oleh</label><h6>';
                            }
                            if($row->approve_by == $this->session->userdata['isLog']['child']){
                                echo "Anda";
                            }  else{
                                $hrd = $this->model_app->view_where('hrd',array('id'=>$row->approve_by));
                                if($hrd->num_rows() > 0){
                            ...
                            if($row->approve_at != null){
                                echo ' <small>('.tanggalwaktu($row->approve_at).')</small>';
                            }
                            echo '</h6></div>';
                        }
                    ?>
        ...
        <div class="card">
            <div class="card-body">
                <h6 class="card-title m-b-15">Aksi</h6>
                <div class="d-flex justify-content-between">
                    <form action="<?= base_url('pengajuan/approve')?>" method="POST">
                        <input type="hidden" name="id" value="<?= encode($row->pengajuan_id)?>">
                        <button class="btn btn-success">Terima</button>
                    </form>
                    <form action="<?= base_url('pengajuan/disapprove')?>" method="POST">
                        <input type="hidden" name="id" value="<?= encode($row->pengajuan_id)?>">
